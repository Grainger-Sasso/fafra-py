...
class LTMMRiskAssessment:
    ...
    def assess_cohort_risk(self):
        ...
        dataset_metrics = self._derive_input_metrics()
        # TODO: feed the dataset metrics into triaining model
        # Train risk model on features
        # Make inference on the cohort
        # Output inferences to csv

    def _derive_input_metrics(self):
        ...
        for ltmm_data in self.ltmm_dataset.get_dataset():
            single_data_metric = {}
            single_data_metric['id']: ltmm_data.get_name()
            # Get largest peak location of walking fft for vertical axis
            largest_fft_peak = self._find_largest_fft_peak(ltmm_data)
            # Get RMS of three acceleration axis
            rms = self._get_triaxial_rms(ltmm_data)
            single_data_metric['fft_peak'] = largest_fft_peak
            single_data_metric['rms'] = rms
    ...
    def _get_triaxial_rms(self, ltmm_data):
        v_axis_data = ltmm_data.get_data().T[0]
        ml_axis_data = ltmm_data.get_data().T[1]
        ap_axis_data = ltmm_data.get_data().T[2]
        return self.motion_filters.calculate_triaxial_rms(v_axis_data, ml_axis_data, ap_axis_data)
    ...
    def _find_largest_fft_peak(self, ltmm_data):
        ...
        fft_v_axis_acc_data = fft.perform_fft(v_axis_acc_data, sampling_rate)
        return peak_detector.detect_peaks(fft_v_axis_acc_data)[0]
