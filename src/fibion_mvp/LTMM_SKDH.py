...
def run_ltmm_skdh(data_path, output_path):
    # Read in LTMM data into numpy array
    data, time = read_ltmm_file(data_path)
    # Generate SKDH pipeline
    pipeline = generate_pipeline(output_path)
    # Run data through the pipeline
    run_pipeline(data, time, pipeline)
    pass


def run_pipeline(data, time, pipeline):
    pipeline.run(time=time, accel=data, fs=fs)


def generate_pipeline(output_path):
    pipeline = Pipeline()
...
def main():
    ...
    run_ltmm_skdh(data_path, output_path)
