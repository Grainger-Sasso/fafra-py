import numpy as np
from src.risk_classification.input_metrics.metric_names import MetricNames
from src.risk_classification.input_metrics.risk_classification_input_metric import RiskClassificationInputMetric
 

METRIC_NAME = MetricNames.ZERO_CROSSING


class Metric(RiskClassificationInputMetric):
    def __init__(self):
        super().__init__(METRIC_NAME)

    def generate_metric(self, **kwargs):
        return self._find_zero_crossing(kwargs['data'],kwargs['enable_mean_crossing'])
    
    def _find_zero_crossing(self, data,enable_mean_crossing = 0):
        """
        zero crossing
        :param data:input would be a 1D array that indicates the data array
        :param sampling_frequency:would be a single float number
        :return:the zero crossing or mean crossing for an array
        """
        data_array = np.array(data)
        real_len = len(data_array)-1
        if real_len < 1:                #perform error checking if input data too short
            raise TooFewDataError
        sampling_frequency=1/len(data_array)
        
        if enable_mean_crossing:        #switch to mean crossing calculation
            data_array = data_array-np.mean(data_array)
        result = data_array[1:len(data_array)]*data_array[0:real_len]           #calculte if there is zero crossing by multiply nearby elements
        for idx, elem in enumerate(result):                 #count zero crossing frequency
            if elem >= 0:
                result[idx] = 0 
            else:
                result[idx] = 1
        result = result.sum()/((1/sampling_frequency)-1)                        #find final crossing rate by dividing time internal
        return result
class Error(Exception):
    """Base class for other exceptions"""
    pass
class TooFewDataError(Error):
    """Raised when input data array has too few elements"""
    pass

        
